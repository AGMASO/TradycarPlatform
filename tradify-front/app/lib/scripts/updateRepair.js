...
export default async function updateRepair(
  ...
  console.log("We are in script updateRepair");
  ...
  try {
    ...
    const tx = await bmw_3er.updateRepairLog(_tokenId, _repairUri);

    // Wait for the first transaction (minting) to be confirmed
    await tx.wait();

    console.log("success to update the Repair Log");
  } catch (error) {
    console.error("Error updating Repair Log:", error);
  }
}
