...
export default function RepairUpdateNft({
  status,
  tokenId,
  addressContractNft,
}) {
  ...
  const [stateOdo, actionOdo] = useFormState(actionUpdateOdo, {
    message: "",
    odoUri: "",
  });
  const [stateRepairLog, actionRepairLog] = useFormState(
    actionUpdateRepairLog,
    { message: "", repairUri: "" }
  );
  const [stateAccidents, actionAccidents] = useFormState(
    actionUpdateAccidents,
    { message: "", accidentUri: "" }
  );
    ...
    <>
      <div className=' flex flex-col min-h-[100vh] justify-center items-center gap-9'>
        <div className=' flex flex-col justify-center items-center '>
          <p className=' text-xl'>
            Introduce the required data to update each section
          </p>
          {status === "Authorized RepairShop" && (
            <p className=' text-green-500 text-xs'>
              You are an Allowed Repair Shop
            </p>
          )}
        </div>
