...
export default function SellerOffersCard({
  id,
  buyer,
  price,
  nftAddress,
    ...
    <Card className='py-4 min-h-[30vh] w-[20vw]' key={id}>
      <CardHeader className='pb-0 pt-2 px-4 flex-col items-center'>
        <h4 className='font-bold text-medium'>
          Buyer with Address: {buyer.slice(0, 5) + "..." + buyer.slice(-5)}
        </h4>
      ...
      <CardFooter className='flex flex-row justify-evenly align-middle'>
        <Button color='danger' variant='ghost' onClick={handleReject}>
          Reject
        </Button>

        <Button color='secondary' variant='ghost' onClick={handleApprove}>
          Accept Offer
        </Button>
      </CardFooter>
    </Card>
